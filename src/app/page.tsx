...
export default function DataClassificationPage() {
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (e) => {
        ...
        if (columnNames.length === 0) {
            toast({ title: "No column names found", description: "The .txt file seems to be empty.", variant: "destructive" });
            return;
        }
  ...
  const classifyAndAddColumns = async (columnNames: string[]) => {
      ...
      if (classifiedColumns.length > 0 && dbConnectionStatus === "connected" && dbUrl) {
          try {
            ...
            complete: async (results) => {
                ...
                if (dbConnectionStatus === "connected" && dbUrl) {
                    ...
                    setColumns(prev => {
                        ...
                        return newColumns.sort((a,b) => a.columnName.localeCompare(b.columnName));
                    });
  ...
  const handleTestConnection = async (urlToTest?: string, dbTypeToTest?: DatabaseType) => {
    ...
    if (!currentUrl) {
      ...
      toast({ title: "Connection Error", description: "Database URL cannot be empty.", variant: "destructive" });
      return;
    }
  ...
  const handleDeleteAll = async () => {
    if (dbConnectionStatus === "connected" && dbUrl) {
      try {
        ...
        const err = error as Error;
        toast({ title: "Network Error", description: `Could not connect to the server to delete data: ${err.message}`, variant: "destructive" });
      }
    } else {
      setColumns([]);
            ...
            <Popover open={isDbPopoverOpen} onOpenChange={setIsDbPopoverOpen}>
              ...
              <PopoverContent className="w-80 space-y-4 p-4 mr-2">
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Supported: PostgreSQL, Oracle, Hive. <br/>(Full features on PostgreSQL & Oracle)
                  </p>
                </div>
                <div className="relative">
                  <Label htmlFor="db-url-input" className="text-sm font-medium">Connection URL</Label>
                  {detectedDbType !== 'unknown' && (
                    <Badge variant="outline" className="absolute right-0 top-0.5 text-xs capitalize">
                      {detectedDbType}
                    </Badge>
                  )}
                  <Input
